...
top.learun = (function ($) {
    ...
    var learun = {
        ...
        // 检测数据是否选中
        checkrow: function (id, msg) {
            var isOK = true;
            if (id == undefined || id == "" || id == 'null' || id == 'undefined') {
                isOK = false;
                var text = '您没有选中任何数据项,请选中后再操作！';
                text = (msg == undefined || msg == "" || msg == 'null' || msg == 'undefined') ? text : msg;
                top.learun.language.get(text, function (text) {
        ...
        //检测变量是否为空
        isNullOrSpace: function (param) {
            if (param == undefined || param == "" || param == 'null' || param == 'undefined') {
                return true;
            }
            return false;
        },
        convertNumber: function (param) {
        ...
        //查询子表数据
        querySubTbData: function (sql, param) {
            ...
            dfop = $.extend(dfop, param);
            var strWhere = '';
            var retdata = [];
            for (var Id in dfop.queryParam) {
                if (!learun.isNullOrSpace(dfop.queryParam[Id])) {
                    strWhere += " and " + Id + " like '%" + dfop.queryParam[Id] + "%'";
                }
            }
            sql = sql + strWhere;
            var url = top.$.rootUrl + '/Common/LoadFormDataBySpeciName';
            learun.httpPost(url, { sql: sql }, function (data) {
                var JsonData = JSON.parse(data.data);
                var bindIdArray = learun.uniqueArray(JsonData);
                var filterdata = [];
                for (var i = 0; i < dfop.rowdatas.length; i++) {
                    for (var j = 0; j < bindIdArray.length; j++) {
                        if (bindIdArray[j]["BindId"] === dfop.rowdatas[i]["Id"]) {
                            filterdata.push(dfop.rowdatas[i]);
                        }
                    }
                }
                retdata = filterdata;
