...
namespace Learun.Util.Operat
{
    ...
    public class OperatorHelper
    {
        ...
        public string GetCrossToken() 
        {
            string token = Guid.NewGuid().ToString();
            string cookieMark = WebHelper.GetCookie(LoginUserMarkKey).ToString();
            string cookieToken = WebHelper.GetCookie(LoginUserToken).ToString();
            if (string.IsNullOrEmpty(cookieMark) || string.IsNullOrEmpty(cookieToken))
            {
                token = "";
            }
            else 
            {
                ...
                redisCache.Write(token + LoginUserMarkKey, cookieMark, ts, CacheId.loginInfo);
                redisCache.Write(token + LoginUserToken, cookieToken,ts, CacheId.loginInfo);
            }
            return token;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public bool WriteCrossToken(string crossToken)
        {
            bool result = false;
            string cookieMark = redisCache.Read<string>(crossToken + LoginUserMarkKey, CacheId.loginInfo); ;
            string cookieToken = redisCache.Read<string>(crossToken + LoginUserToken, CacheId.loginInfo);
            if (!string.IsNullOrEmpty(cookieMark) && !string.IsNullOrEmpty(cookieToken))
            {
                WebHelper.WriteCookie(LoginUserMarkKey, cookieMark);
                WebHelper.WriteCookie(LoginUserToken, cookieToken);
                redisCache.Remove(crossToken + LoginUserMarkKey, CacheId.loginInfo);
                redisCache.Remove(crossToken + LoginUserToken, CacheId.loginInfo);
                result = true;
            }
            return result;
        }
