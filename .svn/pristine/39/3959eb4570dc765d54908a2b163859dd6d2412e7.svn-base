            ...
            try
            {
                ...
                foreach (TNRD_Project_DatailsEntity item in ProjectList)
                {
                    if (!ht_dept.ContainsKey(item.Dept_Code))
                    {
                        ...
                        {
                            var year_single = item_year.BeginTime.ToDate().Year;
                            if (!ht_year.ContainsKey(year_single))
                            {
                                ...
                                node_year.id = year_single+"年".ToString();
                                node_year.text = year_single + "年".ToString();
                                node_year.value = year_single + "年".ToString();
                                node_year.hasChildren = true;
                                node_year.parentId= item.Dept_Code;
                                node_year.complete = true;
                                //根据年份获取项目
                                var itemList = yearList.ToList().Where(u => u.BeginTime.Value.Year == year_single).ToList();
                        ...
                        node_dept.value = item.Dept_Code;
