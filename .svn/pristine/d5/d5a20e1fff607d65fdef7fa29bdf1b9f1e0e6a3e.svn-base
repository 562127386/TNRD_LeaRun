    ...
    public class ModuleBLL : ModuleIBLL
    {
        ...
        public List<ModuleEntity> GetModuleList()
        {
            try
            {
                ...
                {
                    string objectIds = userInfo.userId + (string.IsNullOrEmpty(userInfo.roleIds) ? "" : ("," + userInfo.roleIds)) + (string.IsNullOrEmpty(userInfo.departmentId) ? "" : ("," + userInfo.departmentId));
