...
namespace Learun.Application.Web.Areas.LR_SystemModule.Controllers
{
    ...
    public class AnnexesController : MvcControllerBase
    {
        ...
        #region 通过bootstrap file input 上传文件
        ...
        public JsonResult UploadBFI()
        {
            ...
            try
            {
                ...
                if (file != null)
                {
                    ...
                    string filename = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "_" + oldFileName; //文件重命名
                    string savePath = Server.MapPath(string.IsNullOrEmpty(modelName) ? @"\Content\Files\" + DateTime.Now.ToString("yyyyMMdd") + @"\" : @"\Content\Files\" + modelName + @"\" + DateTime.Now.ToString("yyyyMMdd") + @"\");//Server.MapPath 获得虚拟服务器相对路径
                    ...
                    if (uploaded)
                    {
                        ...
                        ht_result.Add("path", (string.IsNullOrEmpty(modelName) ? @"\Content\Files\" + DateTime.Now.ToString("yyyyMMdd") + @"\" : @"\Content\Files\" + modelName + @"\" + DateTime.Now.ToString("yyyyMMdd") + @"\") + filename);
