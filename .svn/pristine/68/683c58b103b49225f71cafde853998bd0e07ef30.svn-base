...
(function ($, learun) {
    ...
    $.extend(learun, {
        ...
        layerInitForm: function (option) {
            var configData = {
                layerWidth: 600,//弹出层宽度
                layerHeight: 400,//弹出层高度
                ...
                success: null,//确认按钮方法
                select: null,//选中行事件
                unSelect: null//撤销选中行事件
            };
            configData = $.extend(configData, option || {});
            top.lrGirdLayerEdit = {
                height: configData.layerHeight,
                width: configData.layerWidth,
                isMultiselect: configData.isMultiselect,
                colData: configData.colData,
                url: configData.url,
                param: configData.param,
                select: configData.select,
                unSelect: configData.unSelect
            },
                learun.layerForm({
                    id: 'lrlayerform',
                    title: '选择' + configData.title,
                    url: top.$.rootUrl + '/Utility/JfLayerForm',
                    height: configData.gridHeight,
                    width: configData.gridWidth,
                    callBack: function (id) {
                        var res = top[id].acceptClick(function (data) {
                            configData.success(data);
                        });
