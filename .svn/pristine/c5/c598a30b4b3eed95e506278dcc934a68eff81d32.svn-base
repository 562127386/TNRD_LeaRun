...
namespace Learun.Application.Web
{
    ...
    public class HandlerLoginAttribute : AuthorizeAttribute
    {
        ...
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            if (res != 1)// 登录过期或者未登录
            {
                if (CorssSystemAuthorize(filterContext))
                {
                    return;
                }
                else 
        ...
        private bool CorssSystemAuthorize(AuthorizationContext filterContext) 
        {
            bool result = false;
            string crossToken = filterContext.HttpContext.Request["crossToken"];
            if (!string.IsNullOrWhiteSpace(crossToken)) 
            {
                if (OperatorHelper.Instance.WriteCrossToken(crossToken))
                {
                    //去掉crossToken参数并跳转
