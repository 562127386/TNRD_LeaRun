(function ($, learun) {
    ...
    var _jfgrid = {
        ...
        reload: function ($self, op) {
            ...
            if (op.isPage) {
                ...
                learun.httpAsync('GET', op.url, op.param, function (data) {
                    ...
                    if (op.striped) {
                        //$(".jfgrid-data-cell").each(function () {
                        //    if ($(this).attr('rowindex') % 2 != 0) {
        ...
        clickRow: function ($self, $et, op) {
            ...
            $.each(op.running.headData, function (i, col) {
                ...
                if (col.data.edit && col.data.edit.type != 'layer') {
                    ...
                    isEdit = true;
                    var _top = rowIndex * op.rowHeight + 'px';
                    var _left = col.$cell.css('left');
                    var _width = col.$cell.css('width');
            ...
            if (!isEdit) {
                if (op.isMultiselect) {
                    ...
                    if ((_cbValue + '') == '1') {
                        ...
                        op.onUnSelectRow && op.onUnSelectRow(rowData, false);
        ...
        expandSub: function ($self, $et, op) {
            //关闭兄弟节点展开界面
            //if ($et.parent().has('i.fa-minus')) {
            //    var $siblingEach = $et.parent().siblings().has('i.fa-minus');
            //    $siblingEach.each(function () {
            //        var siblingIndex = op.id + '_sub_' + $(this).attr('rowindex');
            //        var $sibling = $('#' + siblingIndex);
            //        $sibling.remove();
            ...
            if (rowItem['jfsubGrid'].value) {
                ...
                for (var i = rowIndex + 1, l = op.running.rowdata.length; i < l; i++) {
                    ...
                    $.each(item, function (_index, _item) {
                        if (_item.top != undefined) {
                            _item.top = _item.top - (op.subGridHeight * op.subGridCount);
                            _item.$cell.css({ 'top': _item.top });
                        }
                    });
                }
                $left.css({ 'height': height - (op.subGridHeight * op.subGridCount) });
                $right.css({ 'height': height - (op.subGridHeight * op.subGridCount) });
            }
            else {
                ...
                // 调整单元格位置
                for (var i = rowIndex + 1;i < op.running.rowdata.length; i++) {
                    //for (var i = rowIndex + 1, l = op.running.rowdata.length; i < l; i++) {
                    var item = op.running.rowdata[i];
                    $.each(item, function (_index, _item) {
                        if (_item.top != undefined) {
                            _item.top += (op.subGridHeight * op.subGridCount);
                            _item.$cell.css({ 'top': _item.top });
                        }
                    });
                }
    ...
    $.fn.jfGrid = function (op) {
        ...
        var dfop = {
            ...
            onUnSelectRow: false,           // 撤销选中一行后回调函数
            dblclick: false,
    ...
    $.fn.jfGridGet = function (name) {
        ...
        switch (name) {
            case 'rowdata':
                ...
                break;
            case 'rowIndex':
                var rownum = $self.find('.jfgrid-data-cell:eq(0)').attr('rownum');
                var rowIndex = parseInt(rownum.replace(op.id + '_', ''));
                return rowIndex;
                break;
            case 'rowdatas':
