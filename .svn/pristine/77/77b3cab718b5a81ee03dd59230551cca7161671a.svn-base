...
(function ($, learun) {
    ...
    learun.frameTab = {
        ...
        closeRefreshTab: function () {
            //获取父页面Id
            var parentId = "lr_iframe_" + top.$('#lr_tab_' + learun.frameTab.iframeId).attr('parent-id');
            //关闭当前页
            learun.frameTab.close(learun.frameTab.iframeId);
            learun.frameTab.focus(parentId.replace("lr_iframe_", ""));
            top.frames[parentId].contentWindow.refreshGirdData();
        },
        closeCurrentTab: function () {
            var currentId = learun.frameTab.iframeId;
            learun.frameTab.close(currentId);
        },
        opencallback: false
        , closecallback: false
    };
